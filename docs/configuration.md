...
The version 2 record binds the durable home and workspace ids to the persistent Herdr session directory's filesystem incarnation, so it remains authoritative across stop/start and presentation-only workspace renames but becomes invalid after the named session is deleted and recreated.
