...
During transition, exactly one live legacy `2ndmate-<id>` workspace is reused without renaming, so its tasks remain reachable for spawn, recovery, send, inspection, cleanup, and presentation ordering.
