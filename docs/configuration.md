...
The version 3 record binds the durable home and workspace ids to a random per-session token anchored to Herdr's persistent `session.json`, so it remains authoritative across stop/start and presentation-only workspace renames but becomes invalid after any named or default session is deleted and recreated.
