...
A herdr task additionally records `herdr_session=`, `herdr_workspace_id=`, `herdr_tab_id=`, and `herdr_pane_id=`; new ordinary tasks also record `herdr_layout=task-workspace`.
...
Every ordinary Herdr ship/scout task gets one workspace labeled `fm-<id>`, created at the exact leased Treehouse worktree and using the workspace's seeded root tab/pane.
Persistent secondmate supervisors keep the home-derived `2ndmate-<secondmate-id>` workspace, while list-live and recovery verify ordinary tasks through the exact workspace/tab/pane identity recorded in that home's metadata; [`docs/herdr-backend.md`](herdr-backend.md) owns the full compatibility and teardown contract.
...
For the herdr backend, `FM_HOME` scopes metadata/recovery and still derives persistent secondmate supervisor labels; ordinary task workspace labels derive from the task id.
