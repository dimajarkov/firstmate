...
The version 3 record binds the durable home and workspace ids to one atomically initialized token stored in the Herdr session directory, so ordinary `session.json` replacement, stop/start, and presentation-only workspace renames preserve authority while deleting and recreating a named session removes the token and invalidates its records.
