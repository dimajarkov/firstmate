...
A secondmate home uses the standard display label `2🏴‍☠️-<id>`, derived from its validated `.fm-secondmate-home` marker with only a terminal `-secondmate` omitted.
The durable marker id remains the routing identity, and the display label never becomes endpoint or parent authority.
The secondmate process and every child it launches resolve the same home identity; a secondmate launched by the primary receives a narrowly scoped home override during container creation.
...
The primary compatibility path accepts exactly one workspace labeled `firstmate` and refuses an ambiguous duplicate.
Each secondmate home instead publishes one immutable identity generation under its own `state/`, binding the durable marker id, a random 128-bit token, the physical named-session directory incarnation, and the exact response-derived workspace id.
Prepared and bound records are mode `0400`, old generations remain read-only, and a replacement generation can create a workspace only after every exact older endpoint is absent or positively agent-free.
This exact binding keeps homes such as `foo` and `foo-secondmate` isolated even though both display as `2🏴‍☠️-foo`.
Secondmate workspace labels, task metadata, cwd, tab labels, pane environments, and projected workspaces never establish parent ownership.
...
Without such a record, one uniquely matching canonical legacy `2ndmate-<durable-id>` workspace may be adopted once and immediately bound to its exact id.
Ambiguous canonical labels, current display labels, renamed workspaces, projections, and task metadata never authorize adoption.
The legacy record and visible workspace label remain unchanged.

The per-home workspace is reused while it has task tabs.
Closing its last tab can remove the workspace, and the next spawn creates or resolves a safely bound replacement.
...
For a secondmate child, the exact identity-bound parent workspace id selects the parent while its current or legacy label is retained only for presentation.
...
Real-runtime fixtures that invoke the installed Treehouse CLI must call `tests/treehouse-test-safety.sh` before the first Treehouse command.
That owner binds the temporary repository, config, update-cache suppression, pool state, runtime wrapper, and supported return and exact-pool cleanup to one exact temporary root.
Missing, redirected, ambient-home, or malformed isolation is refused before pool creation, and temporary-root removal waits until every managed pool is empty.
...
tests/treehouse-test-safety.test.sh
```
