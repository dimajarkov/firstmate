...
The caller-facing label remains `fm-<id>`, while the visible workspace title is `fm-<home-tag>-<id>`.
The operational home tag is `firstmate-<hash>` or `2ndmate-<id>-<hash>`, using a stable short hash of the resolved Firstmate root.
This collision-protecting machine selector intentionally does not adopt Herdr's presentation-only `2🏴‍☠️-<id>` convention.
