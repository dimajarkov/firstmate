...
The home label deliberately retains its collision-safe machine identity - `firstmate` for the primary home, or `2ndmate-<id>` when `$FM_HOME/.fm-secondmate-home` contains a secondmate id - and appends a short stable hash of the resolved `FM_ROOT` path.
It does not adopt Herdr's presentation-only `2🏴‍☠️-<id>` workspace convention.
