...
It is off by default, and the last `/calm` choice persists from the primary Firstmate home across Pi session starts and resumes.
Secondmate homes inherit that primary choice and refuse local `/calm` overrides.
A mid-session inherited change takes effect when the next Pi session starts, not in the running transcript.
