...
| `fm-brief.sh`            | Scaffold ship, scout, and secondmate charters with parent-session runtime ownership  |
| `fm-herdr-lab.sh`        | Guard explicit repository-owned Herdr backend-test sessions                          |
| `fm-install-herdr.sh`    | Install CI's exact-version Herdr pin with official asset URL, SHA-256, and protocol checks |
| `fm-install-treehouse.sh`| Install CI's exact-version Treehouse pin for real-Herdr E2E that needs spawn worktrees |
| `fm-herdr-ci-cleanup.sh` | Snapshot and tear down only job-owned `lab-*` or legacy `fm-lab-*` sessions in the Herdr CI lane |
