...
An ordinary worker or scout tab's visible title is exactly its task id.
Because all homes share one session namespace, creation refuses an exact-title collision rather than attaching across tasks; secondmate naming remains unchanged.
...
Verify it works by spawning a trivial task with `--backend zellij` and confirming the task's meta records `backend=zellij`, `session_name=<id>`, `zellij_session=`, `zellij_tab_id=`, and `zellij_pane_id=`; attaching to the session should show the exact `<id>` title.
...
Firstmate uses **one** zellij session (default name `firstmate`, overridable via `FM_ZELLIJ_SESSION`) and **one tab per task**.
Ordinary worker and scout tabs use the validated task id unchanged, including meaningful trailing digits and dates.
Secondmate tabs retain their historical home-scoped title contract.

## Semantic tab titles and collision safety

Zellij does not enforce tab-title uniqueness, and every firstmate home shares the same session namespace.
Before creation, the adapter checks both the new exact semantic title and the old home-scoped title and refuses if either is occupied.
After creation it verifies that exactly one tab carries the title and that its returned tab id is that tab.
If simultaneous creators raced past the precheck, it closes only its own returned tab id and refuses, never the competing tab.
It never reuses or attaches to a duplicate title.
Metadata-routed operations bind the exact `session_name=` to the recorded tab and pane ids, and `list_live` includes an exact title only when this home's metadata binds that same title, tab id, and pane id.
This preserves cross-home isolation without adding a visible owner prefix.

Tabs created before this change retain their old `fm-<home-label>-<id>` title.
The old `fm_backend_zellij_scoped_title` derivation remains compatibility-only, and existing metadata continues to verify and clean those tabs.
An untagged legacy title is accepted only when exactly one live tab carries it; ambiguous matches refuse rather than guess.
...
When the shared selector contract routes a zellij caller through firstmate metadata, it supplies the recorded `session_name=` to the adapter.
New tasks verify the exact semantic title, while old metadata falls back to the historical `fm-<id>` expectation and compatibility title.
...
| Duplicate task check | `zellij action list-tabs --json`, match exact semantic and compatibility titles | Zellij does not enforce uniqueness, so the adapter refuses either collision before creation. |
| Create task tab | `zellij action new-tab --cwd <dir> --name <id>` | Returns the created tab's bare integer id on stdout. The exact validated task id is visible; no `--no-focus` equivalent exists. |
...
| Kill | `zellij action close-tab-by-id <id>` with recorded tab id and expected `session_name=` | Closing a pane alone leaves a ghost tab. Cleanup closes only a tab whose recorded id still matches the exact semantic or compatibility title. |
| Recovery / list-live | `zellij action list-tabs --json`, compatibility-prefix scan plus exact metadata binding | New exact titles are listed only when this home's metadata binds the same title and current tab and pane ids; old scoped titles retain their prefix-based compatibility path. |
...
The installed zellij version recorded for this adapter is 0.44.0.
The portable fake-CLI suite exercised the exact creation and duplicate-refusal command paths:

```text
$ tests/fm-backend-zellij.test.sh
ok - fm_backend_zellij_create_task: refuses cross-task attachment to a duplicate semantic title
ok - fm_backend_zellij_create_task: creates an exact semantic tab and preserves task-date digits
ok - fm_backend_zellij_create_task: simultaneous same-title creation cleans only its own tab and refuses
```

The create assertion recorded this exact argument:

```text
new-tab --cwd /tmp/proj --name lab-arena-carlos-booking-context-loss-fix-20260723
```

The suite also proves legacy scoped-title verification, ambiguous legacy refusal, recorded-tab cleanup, and current-id checks.
A live zellij binary was not available in this verification environment, so this change makes no new live-version claim beyond the existing 0.44.0 adapter evidence.
...
1. Send, capture, and cwd operations call `fm_backend_zellij_target_ready` first, which verifies session existence via the passive `list-sessions` check and verifies the specific terminal pane via `list-panes --json` filtered to `.id == <pane>` and `.is_plugin == false`.
   When the caller reached the pane through recorded metadata, `target_ready` resolves the pane's owning tab and checks it against the exact `session_name=` or its compatibility title before sending, capturing, or reading cwd.
   ...
   Kill uses teardown's recorded `zellij_tab_id` fallback only after `list-tabs --json` proves the tab still matches the expected exact or compatibility title.
...
`fm_backend_zellij_create_task`'s own `list-tabs`-based duplicate check is therefore required.
Because every home shares the tab bar, an occupied exact semantic title is a safe refusal, never a reuse or cross-task attachment.
...
This is why `fm_backend_zellij_kill` resolves the owning tab id from the pane when possible, accepts the recorded `zellij_tab_id` as a fallback, verifies the exact or compatibility title, and calls `close-tab-by-id`.
...
- **A pane can still die after `target_ready` succeeds and before the operation runs.**
  Metadata-routed operations verify the exact or compatibility title and pane id first, but zellij's unconditional exit 0 leaves this narrow time-of-check/time-of-use race.
...
- **No per-home container split**, unlike herdr's workspace-per-home refinement.
  Cross-home same-id creation now stops safely on the exact-title collision rather than adding a visible owner tag or attaching to another task.
- **Exact-title stale recovery is intentionally conservative.**
  Bulk recovery requires this home's recorded tab and pane ids as well as its exact `session_name=`; if that identity cannot be proved, the adapter refuses rather than guessing.
  Legacy scoped titles retain their compatibility recovery path.
