...
- **No per-home container split**: Zellij keeps its shared session and home-scoped tab titles, while Herdr now gives each ordinary task its own worktree-bound workspace (`docs/herdr-backend.md` "Task container shape").
  This is a deliberate simplicity choice per the locked captain decision (D2: "zellij, content unchanged from the report"), not an oversight.
  This is a container-level visual difference only - the cross-home name-collision gap this shared container shape used to carry is closed by home-scoped tab titles, "Home-scoped tab titles" above.
