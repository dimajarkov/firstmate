...
All destructive Herdr verification used `bin/fm-herdr-lab.sh` with a non-default `fm-lab-` name and a byte-identical default-session tripwire.
No ambient `herdr server stop` command is a supported test operation.

Real-runtime fixtures that also acquire Treehouse worktrees use `tests/treehouse-test-safety.sh` before their first Treehouse command.
Its portable regression is:

```sh
tests/treehouse-test-safety.test.sh
```

The matrix covers isolated repository configuration and state, update-cache suppression inside runtime panes, refusal of external or malformed roots, supported return and exact-pool destruction, immutable ambient listings, normal and failing commands, timeout and signal cleanup, parallel acquisitions, and idempotent repeat cleanup.
...
The secondmate identity hardening is pinned by the same real topology fixture plus the portable adapter suite:
...
The current contract uses the presentation label `2🏴‍☠️-<id>` only for display and binds each secondmate home to an exact response-derived workspace id plus named-session directory incarnation.
The portable matrix covers suffix-colliding display labels, renamed parents, malformed and duplicate generations, session recreation, live-old-endpoint refusal, exact legacy v4/v5 migration, the one canonical legacy-label migration exception, serialization, rollback, in-home state symlinks, and the rule that labels and metadata never infer parent ownership.
The real fixture confirms that the exact bound parent keeps secondmate and child endpoints isolated through creation, list-live, and cleanup.
