...
Herdr's durable default container shape is workspace-per-home plus tab-per-task: the primary home uses `firstmate`, while secondmate homes use the standard display label `2🏴‍☠️-<id>` backed by an exact home-owned workspace identity.
Recovery and list-live scope to the exact workspace for the current `FM_HOME`, never to the mutable display label alone.
