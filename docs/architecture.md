...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, current crew state, endpoint probes, presentation identities, normalized read-only runtime metadata, PR/report pointers, scout reports, and secondmate return-channel guidance.
The runtime projection classifies each task worktree's optional `.arena/worktree-runtime.json` as `absent`, `invalid`, `stale`, or `valid` without claiming service health.
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, including runtime status and slug, so bearings and manual fleet reviews consume one structured contract instead of reparsing raw fleet or project files.
...
Herdr is experimental and can be selected explicitly or by runtime auto-detection: Treehouse remains the sole worktree owner, while Herdr presents eligible Treehouse paths without creating, returning, pruning, or removing them, and its full verification is recorded in `docs/herdr-backend.md`.
Herdr creates one parent workspace per home: the primary uses `firstmate`, and secondmates use `2ndmate-<secondmate-id>`.
Tasks under a single-project secondmate whose parent is associated with that project clone use exact-task-id native child workspaces; primary tasks, secondmate agents, legacy parents, and project-less or multi-project homes retain the tab-per-task layout.
Native child teardown closes the identity-verified Herdr presentation before guarded Treehouse return, while legacy tasks and every non-Herdr backend keep their existing lifecycle.
...
On the Herdr backend, a secondmate launch lands in that home's labeled parent workspace.
For a single-project secondmate, the parent is associated with its project clone so eligible crewmates spawned from that home can appear as exact-task-id native child workspaces; all other shapes retain task tabs in the parent.
