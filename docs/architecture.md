...
Herdr gives every ordinary ship/scout task one `fm-<id>` workspace rooted at its Treehouse worktree and reuses the seeded root tab/pane; persistent secondmate supervisors retain their `2ndmate-<id>` workspace, and legacy task tabs remain recoverable through recorded metadata.
...
On the herdr backend, a secondmate launch lands in that secondmate home's labeled supervisor workspace, while every crewmate it spawns gets a distinct `fm-<id>` workspace rooted at that task's Treehouse checkout.
