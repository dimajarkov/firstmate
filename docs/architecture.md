...
Herdr's durable default container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `firstmate`, secondmate homes use the standard visible label `2🏴‍☠️-<id>` with only a terminal `-secondmate` omitted, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
