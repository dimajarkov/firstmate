...
The preference persists from the primary Firstmate home and is inherited by secondmate homes, which refuse local `/calm` overrides.
A mid-session inherited change takes effect when the next Pi session starts, and toggling Calm off in the primary restores ordinary rendering.
