...
[ "$SM_LABEL_REAL" = "2🏴‍☠️-smoketest-sm1" ] || fail "the secondmate workspace's real herdr label should use the standard display convention, got '$SM_LABEL_REAL'"
pass "real herdr: the secondmate-shaped home's workspace uses the standard display label in herdr itself"
...
POST_SM_ID=$(printf '%s' "$POST_LIST" | jq -r --arg l "2🏴‍☠️-smoketest-sm1" '.result.workspaces[]? | select(.label == $l) | .workspace_id')
