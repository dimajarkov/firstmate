...
test_herdr_lab_contract_is_explicit_and_complete() {
  ...
  assert_grep "records every pre-existing Herdr session except the owned lab before provisioning" "$brief" \
    "Herdr lab brief missing the before tripwire"
  assert_grep "verifies the identical identity and state before destructive calls and after teardown" "$brief" \
    "Herdr lab brief missing the after tripwire"
  assert_grep "a stopped \`default\` is valid and must not be started" "$brief" \
    "Herdr lab brief incorrectly requires a running default session"
