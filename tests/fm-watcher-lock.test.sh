...
test_guard_warnings() {
  ...
  grep -E 'repair (missing watcher supervision|a missing or failed watcher cycle)' "$err" >/dev/null \
    || fail "guard banner missing the harness-aware fix command"
  grep -F 'queued wakes pending - drain them' "$err" >/dev/null || fail "guard did not warn about pending queue"
  grep -E 'After draining queued wakes, repair (missing watcher supervision|a missing or failed watcher cycle)' "$err" >/dev/null \
    || fail "guard did not order supervision repair after drain"
