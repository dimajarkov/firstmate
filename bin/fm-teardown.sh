  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = zellij ]; then
        ...
        ( unset FM_ROOT_OVERRIDE; FM_HOME=$home FM_ROOT=$home fm_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "fm-$child_id" "$(meta_value "$child_meta" herdr_workspace_id)" "$(meta_value "$child_meta" herdr_layout)" ) 2>/dev/null || true
      else
        fm_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "fm-$child_id" "$(meta_value "$child_meta" herdr_workspace_id)" "$(meta_value "$child_meta" herdr_layout)" 2>/dev/null || true
      fi
    fi
    if [ "$child_kind" = secondmate ]; then
      child_home=$(meta_value "$child_meta" home)
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  [ -z "$T_ORCA" ] || fm_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "fm-$ID" "$(meta_value "$META" herdr_workspace_id)" "$(meta_value "$META" herdr_layout)" 2>/dev/null || true
...
if [ "$BACKEND" != orca ]; then
  fm_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "fm-$ID" "$(meta_value "$META" herdr_workspace_id)" "$(meta_value "$META" herdr_layout)" 2>/dev/null || true
fi
if [ "$KIND" = secondmate ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
