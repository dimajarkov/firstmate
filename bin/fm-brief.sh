...
'6. The helper records every pre-existing Herdr session except the owned lab before provisioning and verifies the identical identity and state before destructive calls and after teardown.' \
'   A missing, added, or changed protected session is a hard tripwire failure, never a cleanup warning to ignore; a stopped `default` is valid and must not be started.' \
'' \
'Never bypass the helper, even for a read-only lifecycle probe or cleanup after failure.' \
'The fleet may use named sessions; the helper protects the actual pre-existing session inventory rather than assuming `default` is running.')
else
