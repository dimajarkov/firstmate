...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/herdr-presentation-spaces`, `config/calm`, and the one shared captain-preference file `data/captain-shared.md`.
...
A separate literal-content config reread is required when ordinary inherited `config/*` material changes under an already-running secondmate, and for Calm changes when that live secondmate uses Pi.
After each successful ordinary config write, both the locked bootstrap convergence path and mid-session `bin/fm-config-push.sh` use the shared propagation report to build one per-home generation-specific private instruction file from the validated destination post-write bytes for only the ordinary config items that actually changed for that home (`config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/herdr-presentation-spaces`), in deterministic allowlist order.
...
`config/calm` is the sole exception in purpose, not propagation mechanics: it is a Pi-only transcript-presentation preference, never a model, task-execution, or monitoring-delivery setting.
The primary accepts only `on` or `off` after surrounding whitespace; an absent or invalid primary value converges the secondmate copy to absence, which Calm interprets as off, so no stale preference survives.
Its inherited destination is an ordinary, non-symlinked, single-link read-only file; unsafe source or destination artifacts are rejected, and divergent safe copies are quarantined before replacement or removal.
A changed Calm preference joins a generation only when the live secondmate metadata records `harness=pi`, uses Calm-specific framing, and follows the same durable publication, retry, serialization, and bounded-history mechanics as ordinary config rereads.
That framing states that persistence does not change the live transcript and applies only when the next Pi session starts because Pi exposes no supported external live extension reload or toggle API for this propagation path.
Before delivery, an already-queued generation containing Calm is atomically normalized in place: Pi receives the approved Calm framing with every block preserved, while other harnesses receive every ordinary block byte-exact with only Calm and its framing removed; a Calm-only non-Pi generation is consumed without a message.
Claude, Codex, OpenCode, and Grok secondmates receive the inherited file but never a Calm instruction or Calm behavioral framing.
The `/calm` command refuses to persist a local override in a marked secondmate home and explains that the primary Firstmate manages the preference; primary-home `/calm` behavior is unchanged.
