...
A config-reread pointer does not claim that an already-running Pi presentation changed: Pi exposes no supported external live extension reload or toggle API for this propagation path, so the persisted preference applies at that secondmate's next Pi session.
Non-Pi secondmates safely receive the file but do not consume it.
